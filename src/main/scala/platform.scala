...
object PlatformChecks
{
    ...
    def testExtractHeaders( log : Logger, compiler : Compiler, compileType : CompileType, minimalProgram : String, expectHeaders : Set[String], includePaths : Seq[File] = Seq() ) : Boolean =
    {
    ...
    private def tryCompileAndLinkImpl( log : Logger, compiler : Compiler, compileType : CompileType, andLink : Boolean, minimalProgram : String, includePaths : Seq[File], linkPaths : Seq[File], linkLibraries : Seq[String] ) : Boolean =
    {
        IO.withTemporaryDirectory
        { td =>
    ...
    def tryCompile( log : Logger, compiler : Compiler, compileType : CompileType, minimalProgram : String, includePaths : Seq[File] = Seq() ) =
        tryCompileAndLinkImpl( log, compiler, compileType, false, minimalProgram, includePaths, Seq(), Seq() )
        
    def tryCompileAndLink( log : Logger, compiler : Compiler, compileType : CompileType, minimalProgram : String, includePaths : Seq[File] = Seq(), linkPaths : Seq[File] = Seq(), linkLibraries : Seq[String] = Seq() ) =
    ...
    def testForSymbolDeclaration( log : Logger, compiler : Compiler, compileType : CompileType, symbolName : String, additionalHeaders : Seq[String], includePaths : Seq[File] = Seq(), linkPaths : Seq[File] = Seq(), linkLibraries : Seq[String] = Seq() ) =
    {
    ...
    def testForTypeSize( log : Logger, compiler : Compiler, compileType : CompileType, typeName : String, typeSize : Int, additionalHeaders : Seq[String] = Seq(), includePaths : Seq[File] = Seq() ) =
    {
        val headerIncludes = additionalHeaders.map( h => "#include \"%s\"".format(h) ).mkString("\n")
        val testProg = headerIncludes + "\nstruct TestFoo { unsigned int bf : (sizeof(%s) == %d); };".stripMargin.format( typeName, typeSize )
            
        tryCompile( log, compiler, compileType, testProg, includePaths=includePaths )
    }
    ...
    def requireSymbol( log : Logger, compiler : Compiler, compileType : CompileType, symbolName : String, headers : Seq[String], linkPaths : Seq[File] = Seq(), linkLibraries : Seq[String] = Seq() )
    {
        assert( testForSymbolDeclaration( log, compiler, compileType, symbolName, headers ), "Unable to find required symbol declaration: " + symbolName )
        log.success( "Required symbol is available: " + symbolName )
    }
    
    def requireTypeSize( log : Logger, compiler : Compiler, compileType : CompileType, typeName : String, typeSize : Int, headers : Seq[String] )
    {
        assert( testForTypeSize( log, compiler, compileType, typeName, typeSize, headers ), "Type %s is not of required size %d".format( typeName, typeSize ) )
